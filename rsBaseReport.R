...
epstein9 = c("brisbonl017","mageea058","ajuhanm002","dekreonz027","hindsc050","padillay066","laboye093","sotoo077","wojcika134","parmentierd089","sernad086","johnsonk053","gomesc043")
epstein10 = c("cadenahernandezm046","cardenask152","chavezs134","dasilvac180","figueroae035","ortiz-muriels142","lombak156","riverae083","maybink133")
...
delanos9 = c("gomezn044", "chavezk023", "moored137", "ayalab008", "riganom071", "kinnesl057", "jadachp047", "lemuso118", "restrepoa070")
delanos10 = c("colonias109", "zabb174", "adotevim150", "zuluagas102", "velezm124", "adelekeo001", "girardt043", "aguilarb103", "gomese173", "tavaresa165")

caraza9 = c("agudeloj001","araujog006","armans008", "beaudoinh011","doughtyl027","geraldesj040","santoss073","silvam075","thorntons114")
...
allcock9 = c("belloj136" , "deveigae023" , "davisd140" , "dejesuspereza025" , "fontalvoc038" , "gamboad121" , "hernandeza136" , "jovinc132" , "maganad116" , "morenoj063" , "rojasj113", "vincented120")
allcock10 = c("ardoncantef005", "baross011", "deighanh154" , "diaze031" , "lewise055" , "mingoesc070" , "pinak160" , "tavaresn163")

gibson9 = c("delgadot026","feleyt031","gibbonss041","godineza042","alvarezj084","guzmand095","morine110","wojcikj138","lamoureuxe145")
gibson10 = c("cardonaa019","choquettet024","flanagank037","lincolns056","mckeec065","mensahb069","muneragoezm072","rourket085","valderramal093","malikt112","garcial126")

payette9 = c("dasilvaj022","donisd028","fuentesd036","hernandezc102","khanz126","mejiaa061","millettes088","onifadef142","urzuac082","veigat119")
payette10 = c("castilloj021","cortesm027","costad129","fernandess136","henrriquezw049","mckeej066","melloj113","phillipsd177","sweeneyj090","vasquezj116", "vicentem096", "wieganda099")
...
userTotals = userTotals %>% rowwise() %>% mutate(status = getStatus(totalProgress,grade))
...
testPlot <- function(dfClass, expect9, expect10, grade9Goal, grade10Goal) {
  ...
  if (first(dfClass$grade) == 9){
    plot <- plot + ylim(0,grade9Goal) + geom_hline(aes(yintercept=expect9))
  }
  if (first(dfClass$grade)==10){
    plot <- plot + ylim(0,grade10Goal) + geom_hline(aes(yintercept=expect10))
  }
